...
├── infra/aws/           Terraform: the live deployment's EC2 + networking
└── .github/workflows/   ci.yml (build/test/lint) + deploy.yml (CD to EC2)
```
...
- Tests live in `backend/src/test`, using JUnit 5 + Mockito + AssertJ, one test class per service impl. There's no H2/Testcontainers/`@DataJpaTest` setup, so JPA `Specification` predicate logic (see below) isn't unit-tested — it's verified by hand against a real Postgres instance instead (spin up an isolated `docker run` stack, curl the filters). Keep that verification step if you touch `findAll` filtering; don't assume a Mockito test would have caught a Specification bug.
- `DonationRepository`/`DistributionRepository` extend `JpaSpecificationExecutor` for optional-filter list queries (`GET /donations?startDate=&endDate=`, `GET /distributions?...&institutionId=`) — build the `Specification` in the service layer (see `DonationServiceImpl.findAll`), never in the controller. Add new optional filters the same way: a null check per predicate, `cb.and(predicates.toArray(...))`.
...
- Dashboard metrics beyond simple counts (top donated products, most-attended institutions, the 6-month trend) are computed client-side in `src/utils/aggregate.ts` from `useAllDonations`/`useAllDistributions` (same capped-at-500-records stand-in as the "this month" stats — see the comment on those hooks). The two chart components (`src/components/charts/TopBarList.tsx`, `TrendChart.tsx`) follow this repo's dataviz skill: fixed categorical color order (blue = doações, aqua = distribuições, never reassigned or cycled), one hue per single-series ranked list (never a per-bar value-ramp), rounded-top/square-baseline bars, hairline gridlines, a legend for the 2-series trend chart. Re-run `scripts/validate_palette.js` from the dataviz skill before introducing a third series color.
- The Reports page (`src/pages/ReportsPage.tsx`) is the one place that fetches donations/distributions **filtered on the backend** (`useDonationsReport`/`useDistributionsReport` in the respective hook files — `startDate`/`endDate`/`institutionId` sent as query params, size 2000) rather than aggregating a capped client-side fetch, since report totals need to be exact over an arbitrary date range, not "at a glance."
- PDF/CSV export lives in `src/utils/reportExport.ts` (`exportReportToPdf` via `jspdf`/`jspdf-autotable`, `exportReportToCsv` — plain semicolon-delimited CSV with a UTF-8 BOM, not a real `.xlsx`). **That CSV choice is deliberate, not a shortcut**: the obvious `.xlsx`-writing libraries on npm each carry a real problem — `xlsx` (SheetJS) has a high-severity prototype-pollution/ReDoS CVE with no fix published to npm, `exceljs` pulls in ~90 packages including a moderate-severity `uuid` transitive vuln and several deprecated packages. Run `npm audit` before adding either back, or any other spreadsheet-writing dependency. `ReportsPage.tsx` imports `reportExport.ts` via a **dynamic `import()`** inside the export button handlers (not a static top-level import) specifically because `jspdf` drags in `html2canvas` (~200KB) — static-importing it bloated the main bundle from ~400KB to ~830KB. Keep any future heavy, page-specific dependency (chart export, rich text editors, etc.) behind a dynamic import the same way; check `npm run build`'s chunk size output before and after.

## Deployment (`infra/aws/`)

Full details, tradeoffs, and teardown instructions are in [infra/aws/README.md](infra/aws/README.md) — this is just what an agent needs before touching deploy-related files.

- The live app is one EC2 instance running this repo's own root `docker-compose.yml` via `docker compose up --build -d` — not a separate deployment-specific compose file. If you change `docker-compose.yml`, you're changing what production runs; check `infra/aws/user_data.sh.tpl` (first-boot bootstrap) and `.github/workflows/deploy.yml` (every redeploy) still make sense against your change.
- **`docker-compose.yml`'s `web` port is `${WEB_PORT:-3000}:80`, not a hardcoded `3000:80`.** It was hardcoded originally; the very first real deploy failed because 3000 isn't in the EC2 security group (which opens 80/8080/22) and nobody had reason to open a non-standard port just for this. Production's `.env` (written by `user_data.sh.tpl`) sets `WEB_PORT=80`; local dev is untouched (defaults to 3000). Same pattern as `DB_PASSWORD` — if you add another port or credential that needs to differ between local dev and the deployed instance, parameterize it the same way rather than hardcoding either value.
- **The deploy step does `git fetch && git reset --hard origin/main`, deliberately not `git pull --ff-only`.** The first real CD run failed on exactly this: a manual SSH hotfix (diagnosing the port issue above) left an uncommitted local change on the instance, which a fast-forward-only pull refuses to run past. The instance should always mirror `origin/main` exactly — if you ever SSH in and hand-edit something to unblock yourself, either commit+push that same change immediately or expect the next deploy to silently discard it (that's the intended behavior, not a bug).
- `deploy.yml` triggers on `workflow_run` (after `CI` succeeds on `main`) or `workflow_dispatch` — it does not run on every push directly, so a broken build never reaches the box. If you rename `.github/workflows/ci.yml`'s `name:` field, update the `workflows: ["CI"]` filter in `deploy.yml` to match, or the trigger silently stops firing.
- Required GitHub repo secrets: `EC2_HOST` (the Elastic IP) and `EC2_SSH_KEY` (the private key `infra/aws/generated/*.pem` — gitignored, only exists on whichever machine ran `terraform apply`). Both were set via `gh secret set NAME --body/< file` in this session rather than the web UI, after a web-UI copy/paste attempt saved an empty/wrong value and cost a couple of failed deploy runs — if secrets need setting again and `gh auth status` shows a logged-in session, prefer `gh secret set` over asking a human to paste into a form.
- AWS credentials for Terraform live in `~/.aws/credentials` on whichever machine runs `terraform apply` — never in this repo, never in a chat transcript. If you ever see AWS keys or an EC2 console password appear in a conversation (e.g. a user pasting a screenshot), treat it as compromised and get it rotated before doing anything else with it, even if you personally never "use" the value for anything.
